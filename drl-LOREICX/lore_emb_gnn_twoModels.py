...
f = open('runtimes_icx_omp_nospec2006balanced_spec2006_8classes.pickle', 'rb')
...
for file_VF_IF in files_VF_IF:
    ...
    for k, v in kernel_runtimes.items():
        kernel_runtimes[k] = mean(v)
    label = min(kernel_runtimes, key=kernel_runtimes.get)
    rt_mean = min(kernel_runtimes.values())
    base_mean = mean(base_runtimes[file_VF_IF])
    #print("filename = ", fn)
    #print("VF = ", VF)
    #print("IF = ", IF)
    #print("mean = ", rt_mean)
    vf_if[file_VF_IF] = (rt_mean, label)
    baseline[file_VF_IF] = base_mean
...
for dim in dims:
    emb = {}
    with open('lore_omp_embeddings_256_layer2_001_bs1024.json') as f:
        emb = json.load(f)
    ...
    for num in num_neurons:
        ...
        for kf in range(kfold):
            ...
            #    inp = 6
            model = GCNClassifier(dim, num, 8, g_filenames)
            ...
            train_loader = torch.utils.data.DataLoader(full_data, batch_size=BATCH_SIZE, shuffle = True, collate_fn=collate)
            ...
            for idx in range(len(full_X)):
                ...
                t1 = np.mean(runtimes[fn][int(y)])
                t2 = np.mean(runtimes[fn][VF_pred])
                t3 = baseline[fn]
                t4 = np.mean(runtimes[fn][VF_rand])
            ...
            with open('lore_omp_embeddings2_graphsage_gcn'+str(dim)+'.json', 'w') as f:
                json.dump(emb, f) 
