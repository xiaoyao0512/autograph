...
def smote(data, targets, n_class, n_max, execT, files):
    aug_data = []
    aug_label = []
    aug_exec = []
    aug_file = []
    ...
    for k in range(1, n_class):
        ...
        if smote:
            ...
            for i in range(n_max - class_len):
                ...
                aug_exec.append(execT[row_idx])
                fn = files[row_idx]
                if fn not in map_file:
                    map_file[fn] = 1
                else:
                    map_file[fn] += 1
                aug_file.append(fn+'_'+str(map_file[fn]))
                aug_data.append(new_data.astype('uint8'))
                aug_label.append(k)

    return np.array(aug_data), np.array(aug_label), aug_exec, aug_file
...
def get_smote(dataset,  num_sample_per_class, batch_size, TF_train, TF_test):
    ...
    train_cifar = dataset_(root=DATA_ROOT, train=True, download=False, transform=TF_train)
    #print("train_cifar.targets", train_cifar.targets)
