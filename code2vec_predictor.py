...
from utility import get_snapshot_from_code, get_vectorized_codes
import os, glob
...
def preprocess(vf_if):
    if exists ("features.json"):
        ...
        MAX_LEAF_NODES = 320
        new_rundir = "training_data_default"
        ...
        feat = []
        labels = []
        for current_filename in glob.glob(new_rundir + "/*.c"):
            if ((current_filename == new_rundir + "/header.c") or (current_filename == new_rundir + "/aux_AST_embedding_code.c")):
                continue
            filename = current_filename.split('/')[-1].split('.')[0]
            print(filename)
            files.append(filename)
            labels.append(vf_if[filename][1])
            print(const_orig_codes[current_filename])
            code = get_snapshot_from_code(const_orig_codes[current_filename])
            print(code)
            exit()
...
print("exec2_list = ", exec_list2)
print("exec3_list = ", exec_list3)
#print("bad files = ", bad)
#print("files = ", files)
