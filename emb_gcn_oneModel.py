...
from vocab import full_text_feat
...
def preprocess(vf_if):
    ...
    for f in glob.glob("json-small/*.json"):
        ...
        with open(f) as fh:
            ...
            #feat = []
            #for (n_id, n_deg) in g.degree:
            #    feat.append([n_deg])
            
            
            #feat_new = []
            #for feat_node in feat:
                #feat_node.extend(loop_count[fn_c])
            #    feat_new.append(feat_node)
...
class GraphDataset(Dataset):
    def __init__(self, vf_if):        
        ...
        #self.filenames, self.graphs, self.labels = deep_walk_preprocess(vf_if)
        #self.filenames, self.graphs, self.labels = mfa_preprocess(vf_if)
    def __len__(self):
    ...
    for num in num_neurons:
        ...
        for kf in range(kfold):
            ...
            for idx in range(len(full_X)):
                ...
                #print("probab = ", probab)
                sampled_y = probab.index(max(probab))
                #sampled_y = np.argmax(pred.tolist())
                #sampled_y2 = IF_list[np.argmax(pred2.tolist())]
                #print("fn/predicted/groundtruth = ", fn, sampled_y, y)
                #print("y = ", y)
                ...
                print("label = ", y, (int(sampled_y/5), int(sampled_y%5)))
                if (sampled_y == int(y)):
                    acc += 1
                    files[num].append(fn)
                    pred_list.append(sampled_y)
                ...
                VF_pred = VF_list[int(int(sampled_y) / 5)]
                IF_pred = IF_list[int(sampled_y) % 5]
            ...
            for idx in range(len(full_X)):
                ...
                g.ndata['h'] = h
                hg = dgl.mean_nodes(g, 'h')
                hg = [hg[0].tolist() + model.loops[model.filenames.index(name)].tolist()]  

                emb[name] = hg
                #print(hg)
            with open('embeddings2_gcn_nn.json', 'w') as f:
                json.dump(emb, f) 
