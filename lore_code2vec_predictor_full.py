...
def preprocess_train(vf_if):
    ...
    return feat_train, label_train, file_train


def preprocess_test(vf_if):
    with open("lore_features.json") as f:
        features = json.load(f)
    feats = features["feat"]
    labels = features["labels"]
...
class GraphTrainDataset(Dataset):
    def __init__(self, vf_if):        
        self.feat, self.labels, self.fn = preprocess_train(vf_if)
    def __len__(self):
...
class GraphTestDataset(Dataset):
    def __init__(self, vf_if):
        self.feat, self.labels, self.fn = preprocess_test(vf_if)
    def __len__(self):
        return len(self.labels)
    def __getitem__(self, idx):
        return self.feat[idx], self.labels[idx], self.fn[idx]
...
train_data = GraphTrainDataset(vf_if)
test_data = GraphTestDataset(vf_if)
